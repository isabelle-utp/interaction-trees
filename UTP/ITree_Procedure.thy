...
lemma ret_rbind [simp]: "proc_ret e ;; rbind x = x := e"
  by (simp add: proc_ret_def rbind_def kcomp_itree_def assigns_def expr_defs)
...
lemma hl_proc_seq:
  ...
  using assms
  by (auto elim!:trace_to_bindE bind_RetE' simp add: hoare_rel_triple_def ret_pred_def kcomp_itree_def)
     (metis trace_to_Nil)+
...
lemma hl_proc_call [hoare_safe]: 
  ...
  shows "H{P} x := <C(e)> {Q}"
  using assms
  by (auto simp add: hoare_alt_def proc_call_def kcomp_itree_def)
...
lemma thl_proc_call [hoare_safe]: 
  ...
  apply (auto simp add: proc_call_def taut_def kcomp_itree_def)
  apply (metis (mono_tags, lifting) SEXP_def kcomp_itree_def pre_terminates wp_seq)
  done
...
lemma hl_proc_call_nret [hoare_safe]: 
  ...
  using assms
  by (auto simp add: hoare_alt_def proc_call_nret_def kcomp_itree_def)
...
lemma thl_proc_call_nret [hoare_safe]: 
  ...
  apply (auto simp add: proc_call_nret_def taut_def kcomp_itree_def)
  apply (metis (mono_tags, lifting) SEXP_def kcomp_itree_def pre_terminates wp_seq)
  done
