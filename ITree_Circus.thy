...
definition input_in :: "('a \<Longrightarrow>\<^sub>\<triangle> 'e) \<Rightarrow> 'a list \<Rightarrow> ('a \<Rightarrow> ('e, 's) ktree) \<Rightarrow> ('e, 's) ktree" where
"input_in c A P = (\<lambda> s. inp_in c A \<bind> (\<lambda> x. P x s))"

syntax 
  "_input"    :: "id \<Rightarrow> id \<Rightarrow> logic \<Rightarrow> logic" ("_?'(_') \<rightarrow> _" [90, 0, 91] 91)
  "_input_in" :: "id \<Rightarrow> id \<Rightarrow> logic \<Rightarrow> logic \<Rightarrow> logic" ("_?'(_:_') \<rightarrow> _" [90, 0, 0, 91] 91)

translations "c?(x) \<rightarrow> P" == "CONST input c (\<lambda> x. P)"
translations "c?(x:A) \<rightarrow> P" == "CONST input_in c A (\<lambda> x. P)"
