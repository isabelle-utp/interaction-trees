...
definition promote :: "('s\<^sub>1 \<Longrightarrow> 's\<^sub>2) \<Rightarrow> ('e, 's\<^sub>1) htree \<Rightarrow> ('e, 's\<^sub>2) htree" where
"promote a P = \<questiondown>\<^bold>D(a)? \<Zcomp> frame_ext a P"
