...
state machines are also connected through an event: @{text e3} (from @{text CalSTM} to @{text MoveSTM}).
...
chantype Chan_CalSTM =
  ...
  rec__CalSTM :: "TIDS_CalSTM \<times> InOut \<times> core_int" (* event channels will be renamed to rec_CalSTM (may introduce nondeterminism) *)
  rec_CalSTM :: "InOut \<times> core_int"   (* will be further renamed to rec_Ctrl *)
...
\begin{itemize}
  ...
  \item channels (declared in @{term "Chan_CalSTM"}): @{term "internal_CalSTM"}, @{term "rec_CalSTM"}, etc.
  \begin{itemize}
    ...
    \item Event channels: trigger event channels (@{term "rec__CalSTM"} and @{term "update__CalSTM"}), 
      and action event channels (@{term "rec_CalSTM"} and @{term "update_CalSTM"}).
  \end{itemize}
  ...
  %\item the data type of a channel (@{term "rec_CalSTM"}): @{typeof "rec_CalSTM_C"};
  \item the type of a channel (@{term "rec_CalSTM"}): @{typeof "rec_CalSTM"}, a prism from the data type 
...
"int_int_CalSTM = 
  set ((enumchans3 [rec__CalSTM_C, update__CalSTM_C] [TID_CalSTM_t1,TID_CalSTM_t2] [din, dout] rc.core_int_list) @
...
"CalSTM_s0_triggers = 
  set ((enumchan3 rec__CalSTM_C [TID_CalSTM_t1,TID_CalSTM_t2] [din, dout] rc.core_int_list) @
  ...
  loop (\<lambda> id::integer.
    do {x \<leftarrow> inp_in get_x_CalSTM rc.core_int_set ; 
      (
        ...
        do {inp_in rec__CalSTM (set [(TID_CalSTM_t1, din, l). l \<leftarrow> rc.core_int_list, (x = 0)])
              ; Ret (id)} \<box>
           ...
           (\<lambda> s.
            do {
              ...
              (do {skip ; stop} \<triangle>
                ...
                do {t \<leftarrow> inp_in rec__CalSTM (set [(TID_CalSTM_t1, din, l) . l \<leftarrow> rc.core_int_list]) ;
                ...
                \<comment> \<open> @{text rec__CalSTM} \<close>
                do {
                    x \<leftarrow> inp_in rec__CalSTM (set [(s, d, l) . 
...
true. Otherwise, the first element is false (see the process for the trigger event @{text rec__CalSTM}).
...
definition CalSTM_rec_x_internal_set where
"CalSTM_rec_x_internal_set = 
  set ((enumchan3 rec__CalSTM_C [TID_CalSTM_t1] [din, dout] rc.core_int_list) @ 
...
@{text
  "[
    (rec__CalSTM_C (TID_CalSTM_t0, din, 0), rec_CalSTM_C (din, 0)),
    (rec__CalSTM_C (TID_CalSTM_t1, din, 0), rec_CalSTM_C (din, 0)), 
    ...
  ]"
}
here multiple @{term "rec__CalSTM"} events are mapped to one @{term "rec_CalSTM"} event.
\<close>
definition rename_CalSTM_events where
"rename_CalSTM_events = 
  concat ((enumchan2 (forget_first2 rec__CalSTM_C rec_CalSTM_C TIDS_CalSTM_list) InOut_list rc.core_int_list) @
...
"rename_CalSTM_events_others = 
  ...
  (enumchansp2 [rec_CalSTM_C, update_CalSTM_C] InOut_list rc.core_int_list) @
...
We note the event @{text e3} in the model is not an event of @{text Ctrl}, and we introduce the 
...
"Memory_Ctrl = loop (\<lambda> id::integer.
  ( do {x \<leftarrow> inp_in set_EXT_x_Ctrl rc.core_int_set; 
        ...
        outp set_EXT_x_Ctrl_MoveSTM x; Ret (id)}
  )
)"
...
"rename_Ctrl_CalSTM_events = 
  ...
  (enumchansp2_2 [(rec_CalSTM_C, rec_Ctrl_C), (update_CalSTM_C, update_Ctrl_C)] InOut_list rc.core_int_list)"
...
value. By this way, we connect the output of @{text e3} in @{text CalSTM} to the input of @{text e3} 
in @{text MoveSTM}.
\<close>
...
(* e1 of CalSTM is mapped to it *)
  rec_PatrolMod :: "InOut \<times> core_int"
(* e2 of MoveSTM is mapped to it *)
...
"Memory_PatrolMod = loop (\<lambda> id::integer.
  ( do {x \<leftarrow> inp_in set_x_PatrolMod rc.core_int_set; 
        outp set_EXT_x_PatrolMod_Ctrl x; Ret (id)}
  )
)"
...
"rename_PatrolMod_Ctrl_events = 
  ...
  (enumchanp2_2 (rec_Ctrl_C, rec_PatrolMod_C) InOut_list rc.core_int_list)
"
